...
for(year in year_i) {
  ...
  m = m |>
    addCircleMarkers(data = yeardata, lng = ~lon, lat = ~lat, radius = .5, opacity = 0.8, 
                   ...
                   popup = label_text, group = as.character(year))
}

m = m |> 
  addHeatmap(data = yeardata,lng = ~lon, lat = ~lat, intensity = 2, blur = 5, max = 4, radius = 10, group = "Heatmap Overlay") |> 
  addLegend(labels = legend, colors = palette, title="Ozone levels") |> 
  addLayersControl(overlayGroups = "Heatmap Overlay",
                          baseGroups = as.character(year_i),
